...
class TextMetricsTest {
    ...
    private fun layoutLine(text: String): TextPainter = TextPainter(
        text = TextSpan(text = text, style = TextStyle(fontSize = 20f, fontFamilies = listOf(testFontFamily)))
    ).apply {
    ...
    fun multiline_height_grows_with_line_count() {
        ...
        val multi = TextPainter(
            text = TextSpan(
                text = "line one\nline two\nline three",
                style = TextStyle(fontSize = 20f, fontFamilies = listOf(testFontFamily))
            )
        ).apply {
            layout(0f, 400f)
        }
